...
- [ ] Пересобрать server images и оценить необходимость ротации секретов
  ...
  - Следующий шаг: после поставки пересобрать образы, удалить устаревшие server images штатным процессом и ротировать секреты, если образ был доступен вне доверенного сервера.
...
- [ ] Диагностировать и восстановить текстовые запросы к Groq
  ...
  - Следующий шаг: сначала покрыть HTTP-ответы Groq mock-тестами, затем отдельно выполнить runtime-проверку с действующим ключом владельца и зафиксировать, является ли причина кодом, конфигурацией или отозванным ключом.
...
- [ ] Заменить runtime-DDL полноценными файловыми миграциями
  ...
  - Следующий шаг: внедрить один migration workflow, зафиксировать текущую baseline-схему и проверять миграции перед запуском приложения.

- [ ] Добавить provider-neutral AI-контур и поддержку OpenRouter с выбором модели
  ...
  - Следующий шаг: зафиксировать контракт адаптера и support matrix, добавить OpenRouter, затем contract-тесты успешного JSON-ответа, 401/429/5xx, timeout и некорректного payload.

- [ ] Сделать голосовой ввод Telegram независимым от выбранного текстового AI-провайдера
  ...
  - Следующий шаг: сначала принять support matrix для STT (начальный безопасный вариант — отдельный Groq Whisper credential), затем отделить транскрибацию от LLM-обработки и проверить end-to-end без раскрытия аудио и ключей в логах.
...
- [ ] Заменить AI-поле в списке задач обычным поиском с подсветкой совпадений
...
- [ ] Добавить поиск по категориям в создание и редактирование задачи
  ...
  - Следующий шаг: реализовать доступный searchable combobox поверх уже загруженного `store.categories` и покрыть create/edit/empty/no-match сценарии component-тестами.

- [ ] Добавить страницу канбана и первый режим «Приоритет» с общими фильтрами
  ...
  - Следующий шаг: сначала извлечь общую filter model, затем поставить вертикальный slice route → три колонки → persisted drag → error rollback.

- [ ] Добавить в канбан режим «Категория»
  ...
  - Следующий шаг: переиспользовать общий drag/update механизм, добавив динамическое построение колонок и негативные owner/error tests.

- [ ] Добавить в канбан недельный режим «Дата»
  ...
  - Следующий шаг: до реализации зафиксировать timezone/DST contract и UX для «Без даты»/вне недели, затем покрыть переносы на границе месяца и года тестами.
