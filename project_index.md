...
- **backend/auth.py** - *(412 строк)* 🔗 Зависимости: [hashlib, hmac, random, string, uuid, datetime] `class TelegramAuthData(BaseModel): | class UserRegisterData(BaseModel):`
- **backend/bot.py** - *(758 строк)* 🔗 Зависимости: [logging, os, json, asyncio, io, aiohttp] `class EditTaskState(StatesGroup): | class EditCategoryState(StatesGroup): | def get_task_keyboard(task_id: int) -> InlineKeyboardMarkup:`
- **backend/config.py** - *(17 строк)* 🔗 Зависимости: [os, dotenv] `class Settings:`
- **backend/database.py** - *(132 строк)* 🔗 Зависимости: [datetime, asyncio, orm, sqlalchemy, sql, config] `class UserProfile(Base): | class TelegramLinkCode(Base):`
- **backend/main.py** - *(108 строк)* 🔗 Зависимости: [asyncio, fastapi, cors, contextlib, sqlalchemy, config] `async def lifespan(app: FastAPI):`
- **backend/schemas.py** - *(122 строк)* 🔗 Зависимости: [pydantic, typing, datetime] `class UserProfileUpdate(BaseModel): | class UserProfileResponse(BaseModel): | class ChangePasswordRequest(BaseModel):`
...
- **frontend/src/assets/tasks.css** - *(459 строк)* `.tasks-dashboard { | display: flex;`
...
- **frontend/src/components/tasks/TaskModal.vue** - *(243 строк)* 🔗 Зависимости: [tasks, types] `const props = defineProps<{ | taskToEdit?: Task | null`
- **frontend/src/components/tasks/TasksDashboard.vue** - *(416 строк)* 🔗 Зависимости: [tasks, user, vuedraggable, types, TaskModal, CategoryModal] `[Конфиг / Без сигнатур]`
...
- **frontend/src/views/HomeView.vue** - *(381 строк)* 🔗 Зависимости: [TasksDashboard] `const router = useRouter() | const isAuthenticated = ref(false)`
...
- `POST /telegram-interactive/request` *(в backend/auth.py)*
- `GET /telegram-interactive/status` *(в backend/auth.py)*
