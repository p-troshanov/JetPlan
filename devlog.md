...
## 2026-09-04 — Адаптивные страницы для телефона и планшета

**Работали над:** единым мобильным и планшетным сценарием входа, списка задач, канбана, настроек и модальных форм.

**Изменения:**
- убраны двойные внешние отступы, добавлены fluid-поля и `100dvh`, поэтому страницы используют доступную ширину без горизонтального скролла;
- навигация, панель задач, карточки и настройки перестраиваются под телефон и планшет, а категории остаются видимыми в мобильном списке;
- формы задач и категорий получили внутреннюю прокрутку, safe-area отступы, компактную мобильную раскладку и touch-зоны не меньше 44 px;
- настройки используют общую навигацию рабочего пространства, а иконки мобильных действий получили доступные названия и focus states.
...
- `node --test tests/*.test.ts` — 13 из 13 успешно;
- `node node_modules/vite/bin/vite.js build` — успешно;
- browser smoke-test списка, канбана и настроек на 360×640, 390×844, 768×1024 и 1024×768 — горизонтального overflow нет; мобильные модалки прокручиваются внутри viewport, основные действия доступны.

## 2026-09-04 — Обратная связь настроек и осмысленное раскрытие задач

**Работали над:** понятным результатом сохранения настроек и условием показа кнопки раскрытия длинного описания задачи.

**Изменения:**
- кнопки сохранения показывают состояние запроса, а успешный или ошибочный результат остаётся видимым поверх текущей прокрутки страницы;
- сообщения об успехе уточняют, какой раздел настроек сохранён;
- preview задачи завершается на границе целого предложения и не добавляет кнопку раскрытия, если скрытого текста фактически нет;
- кнопка раскрытия переведена на семантический `button` с клавиатурным focus state и доступными ARIA-атрибутами;
- добавлены unit-тесты коротких, многострочных и длинных описаний.
